...
# --- yr.no / MET Norway: kahden tunnin sadeennuste Porin pisteeseen ---
NOWCAST_URL = ("https://api.met.no/weatherapi/nowcast/2.0/complete"
               "?lat=61.49&lon=21.80")
UA = {"User-Agent": "tornadovahti-pori/1.0 (github.com/tornadovahti-data)"}


def fetch_nowcast():
    """Hakee MET Norwayn nowcastin (Pohjoismaiden tutkat, 5 min valein,
    optical flow -ekstrapolointi). Kirjoittaa out/nowcast.json.
    Epaonnistuminen ei kaada muuta ajoa."""
    try:
        r = requests.get(NOWCAST_URL, headers=UA, timeout=25)
        r.raise_for_status()
        ts = r.json()["properties"]["timeseries"]
        pts = []
        for e in ts[:30]:                      # 30 x 5 min = 2,5 h
            d = e.get("data", {}).get("instant", {}).get("details", {})
            p = d.get("precipitation_rate")
            if p is None:
                continue
            pts.append({"t": e["time"], "p": round(float(p), 2)})
        if not pts:
            print("Nowcast: ei sadetietoa (tutkakatveessa?)")
            return
        os.makedirs("out", exist_ok=True)
        payload = {
            "updated": dt.datetime.now(dt.timezone.utc).isoformat(timespec="minutes"),
            "place": "Pori",
            "source": "MET Norway / yr.no Nowcast 2.0 (CC BY 4.0)",
            "points": pts,
        }
        with open("out/nowcast.json", "w", encoding="utf-8") as f:
            json.dump(payload, f, ensure_ascii=False)
        print(f"Nowcast: {len(pts)} pistetta, huippu {max(x['p'] for x in pts)} mm/h")
    except Exception as e:
        print("Nowcast epaonnistui:", e)
...
def main():
    fetch_nowcast()          # yr.no ensin - toimii vaikka tutka-analyysi ei onnistuisi
